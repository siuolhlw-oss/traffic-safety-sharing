...
// 114 photo case 1
{
  const s = slide(C.ink); page(s, 18, "#BDB9B0"); kicker(s, "照片把動線中斷說清楚", C.white); title(s, "下一步：把標線型人行道的缺口補滿", C.white, 570);
  text(s, "校內人車分流已用紅箭頭標示方向；\n校外 4-4 紀錄仍有住戶門口\n暫不劃設的缺口。", 72, 235, 470, 175, { size: 27, bold: true, color: C.white, lineSpacing: 1.2 });
  text(s, "訪視資料要說實話：\n已完成哪一段？還缺哪一段？", 72, 445, 450, 100, { size: 24, color: C.yellow, lineSpacing: 1.25 });
  await imageOn(s, "school-walkway-marking.jpg", 590, 115, 600, 470, "cover", "竹光國中校內人車分流照片與紅色方向標示");
  source(s, "來源：網站 3-2-1、4-4；紅箭頭為原照片標示，部分校外路段仍有缺口。", "#BDB9B0");
}

// 114 photo case 2
{
  const s = slide(C.white); page(s, 19); kicker(s, "工程不是公文往返，是現場把問題說清楚"); title(s, "先一起走一次，才知道該改哪裡");
  await imageOn(s, "school-site-inspection.jpg", 72, 225, 720, 385, "cover", "竹光國中周邊道路多方現場會勘");
  text(s, "學校", 845, 250, 290, 42, { size: 32, bold: true, color: C.coral });
  text(s, "交通處｜教育處\n里長｜居民", 845, 315, 300, 135, { size: 30, bold: true, lineSpacing: 1.25 });
  box(s, 845, 485, 290, 10, C.yellow);
  text(s, "共同看見，\n才有共同決議。", 845, 520, 300, 80, { size: 25, bold: true, color: C.teal, lineSpacing: 1.2 });
  source(s, "來源：竹光國中 114 交通安全網站 4-4。", C.muted);
}

// 114 photo case 3
{
  const s = slide(C.yellow); page(s, 20); kicker(s, "接送分流不是表格，是每天的現場"); title(s, "人、機車與汽車，需要不同的停等空間", C.ink, 760);
  text(s, "先看現場衝突，\n再決定分流、停等與導護配置。", 72, 270, 610, 125, { size: 32, bold: true, lineSpacing: 1.2 });
  text(s, "照片讓同仁不用靠想像，\n一眼看見尖峰時段的壓力。", 72, 455, 560, 90, { size: 25, lineSpacing: 1.25 });
  await imageOn(s, "school-pickup-zone.jpg", 790, 150, 360, 455, "cover", "竹光國中放學接送區的學生與機車動線");
  source(s, "來源：竹光國中 114 交通安全網站 3-5-1。", C.ink);
}

// 114 photo case 4
{
  const s = slide(C.ink); page(s, 21, "#BDB9B0"); kicker(s, "從駕駛視角看學校周邊常見路況", C.white); title(s, "遠方號誌，會不會吃掉眼前的風險？", C.white, 690);
  await imageOn(s, "school-approach-road.png", 72, 230, 720, 380, "cover", "原簡報第 9 頁的道路駕駛視角");
  text(s, "無號誌路口", 850, 265, 300, 45, { size: 35, bold: true, color: C.yellow });
  text(s, "行人可能就在近端，\n駕駛注意力卻被遠方綠燈拉走。", 850, 350, 300, 145, { size: 27, bold: true, color: C.white, lineSpacing: 1.25 });
  text(s, "這不是只靠提醒就能消失的風險。", 850, 535, 300, 65, { size: 21, color: "#D9D4C9", lineSpacing: 1.2 });
  source(s, "來源：使用者原簡報第 9 頁。", "#BDB9B0");
}
// 114 case 15
{
  const s = slide(C.white); page(s, 22); kicker(s, "資料與成果要分開"); title(s, "這才叫成果：風險條件被改變");
...
// 114 case 16
{
  const s = slide(); page(s, 23); kicker(s, "教授的回饋：肯定做法，也指出下一步"); title(s, "三個優點，三個還要補強的地方");
...
// 114 case 17
{
  const s = slide(C.yellow); page(s, 24); kicker(s, "訪視時由誰報告？原始資料給了意外答案"); title(s, "三次訪視，三種報告者");
...
// 114 case 18
{
  const s = slide(C.ink); page(s, 25, "#BDB9B0"); kicker(s, "我的建議：不要迷信校長一定要上台", C.white); title(s, "誰最適合報告？", C.white);
...
// 114 case 19
{
  const s = slide(C.white); page(s, 26); kicker(s, "事半功倍的核心");
...
// 114 case 20
{
  const s = slide(); page(s, 27); kicker(s, "可直接複製的工作流程"); title(s, "五步驟，把訪視變成平常工作副產品");
  const items=[
    ["01","列問題","通學與事故風險"],
    ["02","借時間","導師課與既有學科"],
    ["03","共用素材","一個資料庫"],
...
// 114 case 21
{
  const s = slide(C.white); page(s, 28); kicker(s, "報告不需要一本書"); title(s, "一張表，只講五件事");
...
// 114 case 22
{
  const s = slide(C.yellow); page(s, 29); kicker(s, "減量清單"); title(s, "五樣東西，不必為訪視重做");
...
// 3
{
  const s = slide(); page(s, 30); kicker(s, "訪視的測量邊界"); title(s, "評鑑測到的，可能只是資料呈現能力");
...
// 4
{
  const s = slide(C.white); page(s, 31); kicker(s, "不是反對評鑑，是要求評鑑回答正確問題"); title(s, "四問評鑑：它有資格代表「安全」嗎？");
  const qs = [
...
// 5
{
  const s = slide(C.ink); page(s, 32, "#BDB9B0"); kicker(s, "教育現場已經 overloading", C.white);
...
// 7
{
  const s = slide(C.yellow); page(s, 34); kicker(s, "重新定義交通安全教育的目的");
...
// 8
{
  const s = slide(C.ink); page(s, 35, "#BDB9B0"); kicker(s, "國際共同語言", C.white); title(s, "Safe System 的六個原則", C.white);
  const data = [
...
// 9
{
  const s = slide(); page(s, 36); kicker(s, "責任必須被看見"); title(s, "教育只是五道防線之一");
...
// 10
{
  const s = slide(C.white); page(s, 37); kicker(s, "教育的邊界");
...
// 11
{
  const s = slide(); page(s, 38); kicker(s, "在地經驗"); title(s, "環境決定人能不能做對", C.ink, 620);
  ...
  await imageOn(s, "slide-10.png", 654, 125, 520, 410, "contain", "原簡報引用的臺北路口降速措施案例：凸島與狹窄部");
  box(s, 672, 560, 500, 10, C.yellow);
  source(s, "案例來源：使用者原簡報第 10–11 頁；圖片為臺北路口降速措施案例。");
}

// 12
{
  const s = slide(C.white); page(s, 39); kicker(s, "國外不是更會宣導，而是更早改變系統"); title(s, "瑞典與荷蘭把安全放進設計");
